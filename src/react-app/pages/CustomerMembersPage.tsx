...
function CreateMemberForm({ onCreated }: { onCreated: () => void }) {
	...
	return <form onSubmit={submit} className="grid gap-4 rounded-xl border bg-card p-4">
		...
		{failed ? <p role="alert" className="text-sm">{t("We could not save the member. Check the information and try again.")}</p> : null}<div><LoadingButton type="submit" loading={pending} loadingLabel={t("Saving…")} disabled={pending || !name.trim() || !branchId}>{t("Save member")}</LoadingButton></div>
	</form>;
}
