...
export function DashboardPage() {
 ...
 const amount = (value: number) => new Intl.NumberFormat(locale, { maximumFractionDigits: 2 }).format(value / 100);
 ...
 return <PageContainer className="space-y-6">
  ...
  {!metrics && !failed ? <div role="status" className="space-y-6"><span className="sr-only">{t("Loading dashboard…")}</span><div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">{[0, 1, 2, 3].map((item) => <div key={item} className="h-44 animate-pulse rounded-2xl bg-muted" />)}</div><div className="h-80 animate-pulse rounded-2xl bg-muted" /></div> : null}
  {metrics && !failed ? <>
   <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
    <Metric icon={<CalendarClock />} label={t("Expected")} value={amount(metrics.expectedMinor)} currency={metrics.currency} to={`/app/charges?period=${period}`} tone="blue" />
    <Metric icon={<ArrowDownLeft />} label={t("Collected")} value={amount(metrics.collectedMinor)} currency={metrics.currency} to="/app/payments" tone="green" />
    <Metric icon={<WalletCards />} label={t("Outstanding")} value={amount(metrics.outstandingMinor)} currency={metrics.currency} to={`/app/charges?period=${period}`} tone="amber" />
   ...
   <div className="grid items-stretch gap-6 xl:grid-cols-[1.5fr_1fr]">
    <section className="rounded-2xl border bg-card p-5 sm:p-7">
     ...
     <div className="my-7 flex flex-col items-center gap-7 sm:flex-row">
      <div role="img" aria-label={`${t("Collection rate")}: ${percent}`} className="relative grid size-40 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(var(--primary) ${Math.max(0, Math.min(1, rate ?? 0)) * 360}deg, var(--muted) 0)` }}><div className="absolute inset-3 rounded-full bg-card" /><div className="relative text-center"><p className="text-3xl font-semibold tracking-tight tabular-nums">{percent}</p><p className="mt-1 text-xs text-muted-foreground">{t("Collection rate")}</p></div></div>
      <dl className="grid w-full gap-5"><div><dt className="flex items-center gap-2 text-sm text-muted-foreground"><span className="size-2 rounded-full bg-primary" />{t("Allocated to this period")}</dt><dd className="mt-1 text-xl font-semibold tabular-nums">{money(metrics.allocatedMinor)}</dd></div><div><dt className="flex items-center gap-2 text-sm text-muted-foreground"><span className="size-2 rounded-full bg-amber-500" />{t("Outstanding")}</dt><dd className="mt-1 text-xl font-semibold tabular-nums">{money(metrics.outstandingMinor)}</dd></div></dl>
     </div>
     <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-5 text-sm"><span className="text-muted-foreground">{t("Expected")}</span><span className="font-semibold tabular-nums">{money(metrics.expectedMinor)}</span></div>
    </section>
