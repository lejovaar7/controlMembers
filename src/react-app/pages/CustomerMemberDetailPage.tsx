...
function EnrollmentSection({ memberId, memberBranchId, plans, enrollments, onChanged }: { memberId: string; memberBranchId: string; plans: Plan[]; enrollments: MemberDetail["enrollments"]; onChanged: () => void }) {
	...
	function editTerms(item: MemberDetail["enrollments"][number]) {
		openDialog({ title: t("Edit future terms"), description: t("Update {plan}. These changes apply to future charges; existing charges will stay the same.", { plan: item.planName }), confirmLabel: t("Save changes"),
			fields: [
				{ name: "amount", label: t("Agreed monthly amount ({currency})", { currency: currencyName(locale, item.currency) }), type: "number", defaultValue: String(item.agreedAmountMinor / 100), min: 0.01, step: 0.01 },
				{ name: "dueDay", label: t("Due day from 1 to 28"), type: "number", defaultValue: String(item.dueDay), min: 1, max: 28 },
				{ name: "discount", label: t("Monthly discount ({currency})", { currency: currencyName(locale, item.currency) }), type: "number", defaultValue: String(item.discountMinor / 100), min: 0, step: 0.01 },
	...
	function submit(event: FormEvent) {
		...
		openDialog({ title: t("Record payment"), description: t("Record this payment with the allocation shown below?"), confirmLabel: t("Record payment"),
			...
			onConfirm: async () => {
				...
				try { const result = await controlMembersApi.createPayment(shell.organizationId, { memberId, branchId, amountMinor, method, paidAt, allocations: preview.allocations.filter((item) => item.amountMinor > 0).map(({ chargeId, amountMinor: allocated }) => ({ chargeId, amountMinor: allocated })), idempotencyKey }); setReceipt(result.receiptNumber); setAmount(""); onChanged(); }
