...
export function CustomerMemberDetailPage() {
	...
	return <PageContainer className="space-y-6">
		<Link to="/app/customer-members" className="inline-flex min-h-10 items-center text-sm font-medium text-muted-foreground hover:text-primary">← {t("Back to members")}</Link>
...
function MemberProfileForm({ member, onSaved }: { member: MemberDetail["member"]; onSaved: () => void }) {
	...
	return <form onSubmit={submit} className="grid gap-4 rounded-xl border bg-card p-4 sm:grid-cols-2">
		...
		<div className="sm:col-span-2"><LoadingButton loading={pending} loadingLabel={t("Saving…")} disabled={pending}>{t("Save member")}</LoadingButton></div>
	</form>;
}
