	...
	return <PageContainer className="space-y-6">
		...
		{workspace ? <>
			...
			<PlansPanel
				...
				plans={workspace.plans.filter((plan) => plan.branchIds.includes(shell.activeBranch?.id ?? ""))}
				tags={workspace.tags}
...
function SettingsForm({ settings, onSaved }: { settings: BillingSettings; onSaved: (value: BillingSettings) => void }) {
	...
	return <form onSubmit={submit} className="grid gap-4 rounded-xl border bg-card p-4 sm:grid-cols-2">
		...
		<div className="flex flex-wrap items-center gap-3 sm:col-span-2"><LoadingButton type="submit" loading={pending} loadingLabel={t("Saving…")} disabled={pending}>{t("Save billing settings")}</LoadingButton>{feedback ? <span role={feedback === "failed" ? "alert" : "status"} className="text-sm">{t(feedback === "saved" ? "Billing settings saved." : "We could not save billing settings.")}</span> : null}</div>
	</form>;
}
...
function PlansPanel({ settings, plans, tags, onCreated, onUpdated }: { settings: BillingSettings; plans: Plan[]; tags: Tag[]; onCreated: (value: Plan) => void; onUpdated: (value: Plan) => void }) {
	...
	const [branchIds, setBranchIds] = useState(() => shell.activeBranch ? [shell.activeBranch.id] : []);
	...
	async function submit(event: FormEvent) {
		...
		try {
			...
			onCreated(created); setName(""); setDescription(""); setAmount(""); setTagNames([]); setTagInput(""); setBranchIds(shell.activeBranch ? [shell.activeBranch.id] : []);
	...
	return <section className="space-y-5 rounded-xl border bg-card p-4">
		...
		<form onSubmit={submit} className="grid gap-4 border-t pt-5">
			...
			<div className="grid gap-4 md:grid-cols-2">
				...
				<div className="grid gap-2"><Label htmlFor="plan-amount">{t("Monthly price ({currency})", { currency: settings.currency ? currencyName(locale, settings.currency) : "—" })}</Label><MoneyInput id="plan-amount" min={0.01} value={amount} required onValueChange={setAmount} /></div>
			...
			<div><LoadingButton type="submit" loading={pending} loadingLabel={t("Saving…")} disabled={pending || !settings.currency || !name.trim() || !amount || branchIds.length === 0}>{t("Add plan")}</LoadingButton></div>
		</form>
	</section>;
}
...
function PlanCard({ plan, onUpdated }: { plan: Plan; onUpdated: (value: Plan) => void }) {
	...
	return <li className="rounded-xl border bg-background p-4">
		...
		{editing ? <form onSubmit={save} className="grid gap-3">
			...
			{failed ? <p role="alert" className="text-sm">{t("We could not save the plan.")}</p> : null}
			<div className="flex flex-wrap gap-2"><LoadingButton type="submit" loading={pending} loadingLabel={t("Saving…")} disabled={pending || branchIds.length === 0}>{t("Save changes")}</LoadingButton><Button type="button" variant="outline" disabled={pending} onClick={() => { reset(); setEditing(false); }}>{t("Cancel")}</Button></div>
		</form> : <>
			...
			<p className="mt-1 text-sm text-muted-foreground">{t("Available at branches")}: {shell.branches.filter((branch) => plan.branchIds.includes(branch.id)).map((branch) => branch.name).join(", ")}</p>
