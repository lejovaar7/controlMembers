...
function PlanCard({ plan, locale, onUpdated }: { plan: Plan; locale: string; onUpdated: (value: Plan) => void }) {
	...
	return <li className="rounded-xl border bg-background p-4">
		{editing ? <form onSubmit={save} className="grid gap-3">
			...
			{failed ? <p role="alert" className="text-sm">{t("We could not save the plan.")}</p> : null}
			<div className="flex gap-2"><Button disabled={pending || branchIds.length === 0}>{t(pending ? "Saving…" : "Save changes")}</Button><Button type="button" variant="outline" disabled={pending} onClick={() => { reset(); setEditing(false); }}>{t("Cancel")}</Button></div>
		</form> : <>
