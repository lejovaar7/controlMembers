	...
	return (
		<PageContainer className="space-y-4">
			...
			{failed ? <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-destructive/20 bg-destructive/5 p-5 text-sm">{t("We could not load users.")}<Button variant="outline" onClick={() => setRevision((value) => value + 1)}>{t("Try again")}</Button></div> : current?.members.length === 0 ? <p className="rounded-2xl border border-dashed bg-card p-10 text-center text-sm text-muted-foreground">{t("No users found.")}</p> : (
				<UsersTable members={current?.members ?? []} branches={shell.branches} isOwner={shell.organizationRole === "owner"} loading={!current} actions={(entry) => <div className="user-action-buttons">
					<Button variant="outline" aria-label={t("Edit access for {name}", { name: entry.user.name })} disabled={Boolean(form)} onClick={(event) => { editButtonRef.current = event.currentTarget; setMessage(null); setForm(entry); }}><Pencil aria-hidden="true" />{t("Edit")}</Button>
					<Button variant="outline" className={entry.isActive ? "text-destructive hover:text-destructive" : undefined} aria-label={t(entry.isActive ? "Deactivate company access for {name}" : "Reactivate company access for {name}", { name: entry.user.name })} disabled={Boolean(form || resending)} onClick={() => changeStatus(entry)}>{entry.isActive ? <Ban aria-hidden="true" /> : <RotateCcw aria-hidden="true" />}{t(entry.isActive ? "Deactivate" : "Reactivate")}</Button>
					{entry.setupRequired && entry.isActive && <LoadingButton loading={resending === entry.membershipId} loadingLabel={t("Sending…")} variant="outline" className="col-span-2" aria-label={t("Resend setup for {name}", { name: entry.user.name })} disabled={Boolean(resending)} onClick={() => void resend(entry)}><Mail aria-hidden="true" />{t("Resend setup")}</LoadingButton>}
				</div>} />
			)}
		</PageContainer>
	);
