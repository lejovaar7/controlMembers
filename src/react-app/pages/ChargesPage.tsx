...
export function ChargesPage() {
	...
	function adjust(item: Charge) {
		openDialog({ title: t("Adjust charge"), description: t("Update the adjustment for {name} · {plan}.", { name: item.memberName, plan: item.planName }), confirmLabel: t("Save changes"),
			fields: [{ name: "amount", label: t("New adjustment amount in {currency}", { currency: currencyName(locale, item.currency) }), type: "number", defaultValue: String(item.adjustmentMinor / 100), min: -(item.totalMinor - item.adjustmentMinor) / 100, step: 0.01 }, { name: "reason", label: t("Reason for this adjustment"), type: "textarea" }],
