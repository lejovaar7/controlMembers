...
function PaymentComposer({ detail, currency, chargeId, returnFocus, onClose, onSaved, onChangeMember }: Props & { detail: MemberDetail; currency: string; onChangeMember?: () => void }) {
	...
	const payable = detail.charges.filter((charge) => charge.branchId === branchId && charge.lifecycle === "open" && charge.outstandingMinor > 0)
		.sort((left, right) => left.dueDate.localeCompare(right.dueDate) || left.id.localeCompare(right.id));
	const target = payable.find((charge) => charge.id === chargeId);
	const initial = chargeId ? target?.outstandingMinor ?? 0 : memberPaymentDefault(detail, branchId);
	const [amount, setAmount] = useState(initial > 0 ? String(initial / 100) : ""); const [method, setMethod] = useState("cash"); const [destination, setDestination] = useState(chargeId ?? payable[0]?.id ?? "oldest");
	...
	function resetPreview() { setAllowCredit(false); setPreview(null); setPreviewFailed(false); setError(null); }
	function changeDestination(value: string) {
		const suggested = value === "oldest" ? payable.reduce((sum, item) => sum + item.outstandingMinor, 0) : payable.find((item) => item.id === value)?.outstandingMinor ?? 0;
		setDestination(value); setAmount(suggested > 0 ? String(suggested / 100) : ""); resetPreview();
	}
	...
	function review(event: FormEvent) {
		event.preventDefault(); if (!preview || !validPreview || !validAmount || !validMethod || (preview.creditMinor > 0 && !allowCredit)) return;
		setError(null); setAttempt({ input: { ...(preview.creditMinor > 0 ? { allowCredit: true } : {}), memberId, branchId, amountMinor, method, paidAt: new Date().toISOString(), allocations: preview.allocations.filter((item) => item.amountMinor > 0).map(({ chargeId: id, amountMinor: allocated }) => ({ chargeId: id, amountMinor: allocated })), idempotencyKey: createIdempotencyKey() }, preview, methodName: paymentMethodLabel(methods.methods.find((entry) => entry.id === method)!, t) });
	}
	async function confirm() {
		...
		try { const result = await controlMembersApi.createPayment(shell.organizationId, attempt.input); pendingPaymentAttempts.delete(scope); onSaved(result); }
			...
			if (!(failure instanceof ProductApiError) || failure.status >= 500) { setUncertain(true); setError("We could not confirm the result. Retry this same payment before starting another."); }
				...
				setError(failure.code === "CREDIT_CONFIRMATION_REQUIRED" ? "Confirm the advance explicitly before recording money without a monthly fee." : failure.code === "ALLOCATION_CONFLICT" ? "The balance changed. Review the updated amounts before confirming again." : failure.code === "PAYMENT_METHOD_UNAVAILABLE" ? "Payment method is no longer available. Choose another method." : "We could not save this payment. Review the details and try again.");
				methods.reload();
			}
	...
	return <CenteredDialog open wide focusKey={attempt ? "review" : "entry"} title={t(attempt ? "Review payment" : "Record payment")} description={t("Review the member, branch and amounts before confirming.")} pending={pending} onClose={onClose} returnFocus={returnFocus}>
		<form onSubmit={attempt ? (event) => { event.preventDefault(); void confirm(); } : review} className="space-y-5 p-5 sm:p-7">
			...
			{!attempt && !detail.charges.some((item) => item.branchId === branchId && item.lifecycle === "open" && item.outstandingMinor > 0) ? <p role="status" className="rounded-xl border bg-muted/40 p-4 text-sm">{t(detail.charges.some((item) => item.branchId === branchId) ? "There are no unpaid monthly fees in this branch. Any new money will be an advance, not another payment for a paid fee." : "No monthly fees have been created in this branch. Generate the fee first, or explicitly record an advance.")}</p> : null}
			{attempt ? <div className="space-y-1"><p className="text-2xl font-semibold tabular-nums">{money(attempt.input.amountMinor)}</p><p className="text-sm text-muted-foreground">{attempt.methodName}</p></div> : <fieldset disabled={pending} className="grid gap-4 sm:grid-cols-2">
				<div className="grid min-w-0 gap-2 sm:col-span-2"><Label htmlFor="payment-destination">{t("Monthly fee to pay")}</Label><SelectField id="payment-destination" value={destination} disabled={!payable.length} onValueChange={changeDestination} options={[
					...(chargeId && !target ? [{ value: chargeId, label: t("This fee is no longer available for payment."), disabled: true }] : []),
					...payable.map((item) => ({ value: item.id, label: `${item.planName} · ${formatDate(locale, new Date(`${item.billingPeriod}-01T12:00:00Z`), { month: "long", year: "numeric", timeZone: "UTC" })} · ${money(item.outstandingMinor)}` })),
					{ value: "oldest", label: t(payable.length ? "Several fees, oldest first" : "No unpaid monthly fees available") },
				]} /><p className="text-xs text-muted-foreground">{t("Only unpaid fees created for this member in this branch appear here. The amount updates when you choose a fee.")}</p></div>
