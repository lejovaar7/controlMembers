...
export function MembersTable({ members, currency, loading = false }: { members: CustomerMember[]; currency: string; loading?: boolean }) {
	...
	return <div className="member-directory rounded-2xl border bg-card shadow-sm" aria-busy={loading}>
		...
		<table className="members-table" role="table">
			...
			<thead role="rowgroup"><tr role="row">{headers.map((label, index) => <th key={label} scope="col" role="columnheader" className={index === 4 ? "member-balance" : undefined}>{label}</th>)}</tr></thead>
			<tbody role="rowgroup">
				{loading ? [0, 1, 2, 3].map((row) => <tr key={row} role="row" aria-hidden="true">{headers.map((label, column) => <td key={label} role="cell" data-label={label}><Skeleton className={column === 0 ? "h-4 w-40 max-w-full" : "h-4 w-20 max-w-full"} /></td>)}</tr>) : members.map((member) => <tr key={member.id} role="row" className="member-clickable-row" onClick={(event) => {
