...
export async function createEnrollment(env: Env, request: Request, memberId: string, body: Record<string, unknown>) {
	...
	const dueDay = body.dueDay === undefined ? selectedPlan.plan.defaultDueDay : Number(body.dueDay);
	...
	await db.batch([
		db.insert(enrollment).values({ id, organizationId: tenant.organizationId, memberId, planId, branchId: branch.branchId, startDate, endDate, status: "active", agreedAmountMinor, currency: selectedPlan.plan.currency, dueDay, discountMinor, createdByUserId: tenant.userId }),
		db.insert(auditEvent).values({ id: crypto.randomUUID(), organizationId: tenant.organizationId, eventType: "enrollment.created", actorUserId: tenant.userId, subjectType: "enrollment", subjectId: id, branchId: branch.branchId, detailsJson: details({ memberId, planId, startDate, agreedAmountMinor, dueDay, discountMinor }) }),
	]);
	return { id, memberId, planId, planName: selectedPlan.plan.name, branchId: branch.branchId, startDate, endDate, status: "active", agreedAmountMinor, currency: selectedPlan.plan.currency, dueDay, discountMinor };
}
...
export async function updateEnrollment(env: Env, request: Request, id: string, body: Record<string, unknown>) {
	...
	const dueDay = body.dueDay === undefined ? current.dueDay : Number(body.dueDay);
	...
	if (!Number.isSafeInteger(agreedAmountMinor) || agreedAmountMinor <= 0 || !Number.isInteger(dueDay) || dueDay < 1 || dueDay > 28 || !Number.isSafeInteger(discountMinor) || discountMinor < 0 || discountMinor > agreedAmountMinor || (endDate && endDate < current.startDate)) throw new RequestError(400, "INVALID_INPUT");
	...
	await db.batch([
		db.update(enrollment).set({ status, agreedAmountMinor, dueDay, discountMinor, endDate: finalEndDate, updatedAt: now() }).where(and(eq(enrollment.id, id), eq(enrollment.organizationId, tenant.organizationId))),
		db.insert(auditEvent).values({ id: crypto.randomUUID(), organizationId: tenant.organizationId, eventType: "enrollment.updated", actorUserId: tenant.userId, subjectType: "enrollment", subjectId: id, branchId: current.branchId, detailsJson: details({ before: { status: current.status, agreedAmountMinor: current.agreedAmountMinor, dueDay: current.dueDay, discountMinor: current.discountMinor }, after: { status, agreedAmountMinor, dueDay, discountMinor, endDate: finalEndDate }, reason }) }),
	]);
	return { ...current, status, agreedAmountMinor, dueDay, discountMinor, endDate: finalEndDate };
}
