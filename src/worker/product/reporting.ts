...
function branchCondition<T>(tenant: Awaited<ReturnType<typeof requireTenant>>, column: T) {
	return tenant.allBranches ? undefined : (tenant.branchIds.length ? inArray(column as never, tenant.branchIds) : eq(column as never, ""));
}
...
export async function getDashboard(env: Env, request: Request) {
	...
	const chargeWhere = and(eq(charge.organizationId, tenant.organizationId), eq(charge.billingPeriod, period), branchId ? eq(charge.branchId, branchId) : branchCondition(tenant, charge.branchId));
	...
	const [collected] = await db.select({ amount: sql<number>`coalesce(sum(${payment.amountMinor}), 0)` }).from(payment).where(and(
		...
		branchId ? eq(payment.branchId, branchId) : branchCondition(tenant, payment.branchId),
	));
...
export async function getMemberBalances(env: Env, request: Request) {
	...
	requireReports(tenant);
	const db = getTenantDb(env, tenant.organizationId);
	const members = await db.select({ id: customerMember.id, displayName: customerMember.displayName, branchId: customerMember.primaryBranchId, branchName: team.name, status: customerMember.status })
		.from(customerMember).innerJoin(team, eq(team.id, customerMember.primaryBranchId)).where(and(eq(customerMember.organizationId, tenant.organizationId), branchCondition(tenant, customerMember.primaryBranchId)));
	const memberIds = members.map((item) => item.id);
	const charges = memberIds.length ? await db.select({ id: charge.id, memberId: charge.memberId, totalMinor: charge.totalMinor }).from(charge).where(and(eq(charge.organizationId, tenant.organizationId), eq(charge.status, "open"), inArray(charge.memberId, memberIds))) : [];
	...
	const postedPayments = memberIds.length ? await db.select({ id: payment.id, memberId: payment.memberId, amountMinor: payment.amountMinor }).from(payment).where(and(eq(payment.organizationId, tenant.organizationId), eq(payment.status, "posted"), inArray(payment.memberId, memberIds))) : [];
...
export async function getFinancialReports(env: Env, request: Request) {
	...
	}).from(charge).where(and(eq(charge.organizationId, tenant.organizationId), eq(charge.status, "open"), branchCondition(tenant, charge.branchId)));
	const chargeIds = charges.map((item) => item.id);
	const allocatedRows = chargeIds.length ? await db.select({ chargeId: allocation.chargeId, amount: sql<number>`coalesce(sum(${allocation.amountMinor}), 0)` })
		.from(allocation).innerJoin(payment, eq(payment.id, allocation.paymentId))
		.where(and(eq(allocation.organizationId, tenant.organizationId), eq(payment.status, "posted"), inArray(allocation.chargeId, chargeIds))).groupBy(allocation.chargeId) : [];
	...
	return {
		...
		scope: tenant.allBranches ? "all" : "assigned",
	};
}
...
export async function exportCsv(env: Env, request: Request, kind: string) {
	...
	if (kind === "members") {
		...
		rows = (await db.select({ member: customerMember, branchName: team.name }).from(customerMember).innerJoin(team, eq(team.id, customerMember.primaryBranchId)).where(and(eq(customerMember.organizationId, tenant.organizationId), branchCondition(tenant, customerMember.primaryBranchId)))).map(({ member, branchName }) => [member.id, member.displayName, member.status, branchName, member.documentNumber, member.email, member.phoneE164, member.externalReference]);
	} else if (kind === "payments") {
		headers = ["receipt", "paid_at", "member", "branch", "method", "amount_minor", "currency", "status"];
		rows = (await db.select({ payment, memberName: customerMember.displayName, branchName: team.name }).from(payment).innerJoin(customerMember, eq(customerMember.id, payment.memberId)).innerJoin(team, eq(team.id, payment.branchId)).where(and(eq(payment.organizationId, tenant.organizationId), branchCondition(tenant, payment.branchId), period ? gte(payment.paidAt, zonedMidnight(`${period}-01`, tenant.timezone)) : undefined, period ? lt(payment.paidAt, zonedMidnight(`${addMonth(period)}-01`, tenant.timezone)) : undefined))).map((item) => [item.payment.receiptNumber, item.payment.paidAt.toISOString(), item.memberName, item.branchName, item.payment.method, item.payment.amountMinor, item.payment.currency, item.payment.status]);
	} else if (kind === "receivables") {
		headers = ["charge_id", "period", "due_date", "member", "plan", "branch", "total_minor", "currency", "lifecycle"];
		rows = (await db.select().from(charge).where(and(eq(charge.organizationId, tenant.organizationId), branchCondition(tenant, charge.branchId), period ? eq(charge.billingPeriod, period) : undefined))).map((item) => [item.id, item.billingPeriod, item.dueDate, item.memberNameSnapshot, item.planNameSnapshot, item.branchNameSnapshot, item.totalMinor, item.currency, item.status]);
