...
export type AccessInput = { role: "admin" | "member"; branchIds: string[]; allBranches: boolean; canAppointAdmins?: boolean };
...
export async function validateAccessInput(env: Env, tenant: TenantContext, body: Record<string, unknown>): Promise<AccessInput> {
	...
	for (const key of ["allBranches", "canAppointAdmins"]) {
	...
	return { role: body.role, branchIds, allBranches, ...(typeof body.canAppointAdmins === "boolean" ? { canAppointAdmins: body.canAppointAdmins } : {}) };
}

export async function provisionMember(env: Env, request: Request, tenant: TenantContext, body: Record<string, unknown>) {
	if (Object.keys(body).some((key) => !["name", "email", "role", "branchIds", "allBranches", "canAppointAdmins"].includes(key))) throw new RequestError(400, "INVALID_INPUT");
...
export async function updateMemberAccess(env: Env, request: Request, tenant: TenantContext, membershipId: string, body: Record<string, unknown>) {
	const target = await requireManageableMember(env, tenant, membershipId);
	if (Object.keys(body).some((key) => !["role", "branchIds", "allBranches", "canAppointAdmins"].includes(key))) throw new RequestError(400, "INVALID_INPUT");
	const access = await validateAccessInput(env, tenant, body);
	const auth = getAuth(env);
	const db = getTenantDb(env, tenant.organizationId);
